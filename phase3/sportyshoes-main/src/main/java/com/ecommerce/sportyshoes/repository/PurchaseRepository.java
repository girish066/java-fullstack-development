...
public class PurchaseRepository {
	...
	public Optional<Purchase> findById(int i) throws BusinessException{
		try {
		return Optional.of(jdbcTemplate.queryForObject("select * from purchase_order where id=?", 
				new Object[] {i}, new BeanPropertyRowMapper<Purchase>(Purchase.class)));}
	...
	public int deleteSale(int i) throws BusinessException {
		try {
			return jdbcTemplate.update("delete from purchase_order where id=?", new Object[] {i});}
			catch (Exception exception) {
				throw new BusinessException("Internal Error Occured "+exception.getMessage());
				}
			}
